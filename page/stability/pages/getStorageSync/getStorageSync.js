...
Page({
  ...
  setStorage() {
    my.setStorage({
      ...
      data: 'alipay',
      success() {
        my.alert({ content: '存储数据成功!', });
      },
      fail: (error) => {
        my.alert({ content: '保存数据失败' + JSON.stringify(error), });
      },
    });
  },
  getStorageSync() {
    my.alert({ content: '同步获取数据' + JSON.stringify(my.getStorageSync({ key: '1' })), });
  },
getStorageSync_many(){
 for (var i = 0; i < 30; i++) {
   my.showToast({ content: i+'同步获取数据' + JSON.stringify(my.getStorageSync({ key: '1' })), });
 }
},
getStorageSync_null(){
 my.alert({ content: '同步获取数据' + JSON.stringify(my.getStorageSync({ key: null })), });
},
getStorageSync_number(){
 my.alert({ content: '同步获取数据' +JSON.stringify(my.getStorageSync({ key: 0 })), });
},

getStorageSync_array(){
 my.alert({ content: '同步获取数据' +JSON.stringify(my.getStorageSync({ key: [0,1,2] })), });
},
getStorageSync_symbol(){
 my.alert({ content: '同步获取数据' +JSON.stringify( my.getStorageSync({ key:'%$#'})), });
},
getStorageSync_bool(){
 my.alert({ content: '同步获取数据' + JSON.stringify(my.getStorageSync({ key: true})), });
},
});
